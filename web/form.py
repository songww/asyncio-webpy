...
class Form(object):
    ...
    def render(self):
        ...
        for i in self.inputs:
            html = ''.join([utils.safeunicode(i.pre) ,i.render(), self.rendernote(i.note),
                            utils.safeunicode(i.post)])
            if i.is_hidden():
                out.append(f'    <tr style="display: none;"><th></th><td>{html}</td></tr>\n')
            else:
                out.append(f'    <tr><th><label for="{net.websafe(i.id)}">{net.websafe(i.description)}</label>'
                           f'</th><td>{html}</td></tr>\n')
        out.append("</table>")
        return out
